...
class WLPatternAnalyzer(QMainWindow):
    ...
    def _on_model_changed(self, index):
        ...
        if self.model_combo.count() > 0:
            ...
            if model_name in self.analysis_models:
    ...
    def _perform_analysis(self):
        ...
        # Eğer yeterli veri yoksa veya model yoksa çalıştırma
        if len(self.history) < 5 or not self.current_model:
            self._reset_prediction_ui()
            return

        # Basit bir tahmin yap (modeller yeniden eklendiğinde geliştirilecek)
        prediction = 0  # Belirsiz
        confidence = 0.5  # Orta güven seviyesi
